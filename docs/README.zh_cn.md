...
## 功能
 ...
 - ~bootloader  [仅限骁龙 8 Elite Gen 5 芯片机型(Xiaomi 17 Series & Redmi K90 Pro Max)可运行！不要在其他设备上运行该命令]
                尝试使用efi解锁文件解锁设备Bootloader锁
 - cls          清除屏幕内容
 - debug        切换工具调试模式，为True时，会显示详细的运行信息 
 - devices      获取已连接设备的信息
 - help         显示此帮助消息 
 - safe         切换工具安全模式，为True时，将禁止一些危险操作，比如命令“rm” 
 - selinux      尝试设置selinux为宽限模式
 - shell [COMMAND]      使用 adb shell 运行命令
 - switch [SERIAL]      使用序列号切换到对应的设备
